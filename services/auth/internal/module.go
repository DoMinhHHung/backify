...
func New(ctx context.Context, cfg Config) (*App, error) {
	...
	// JWT_SECRET rỗng phải chặn ngay ở New, không để service chạy được rồi
	// ký token bằng secret rỗng — issuer/verifier tự trả lỗi nếu rỗng
	// (NewIssuer/NewVerifier), ở đây chỉ cần fail nhanh và đóng tài nguyên
	// đã mở giống mọi bước thất bại khác trong New.
	jwtIssuer, err := authjwt.NewIssuer(cfg.JWTSecret)
	if err != nil {
		_ = controlConn.Close()
		subscriber.Close()
		rabbitConn.Close()
	...
	jwtVerifier, err := authjwt.NewVerifier(cfg.JWTSecret)
	if err != nil {
		_ = controlConn.Close()
		subscriber.Close()
		rabbitConn.Close()
